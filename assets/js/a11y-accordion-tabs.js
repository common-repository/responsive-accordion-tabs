...
(function () {
	...
	function AccordionTabs(el, options) {
		...
		} else if (el.getAttribute('data-start-collapsed') === undefined || el.getAttribute('data-start-collapsed') == 'false') {
			this.options.startCollapsed = false;
		}
		
		var hash = window.location.hash;
		if ( hash ) {
			var a_el = document.getElementById( hash.replace( "#", "" ) );
			var url = a_el.href;
			var parts = url.split('#');
			if (parts.length > 1) {
				this.options.selectedTab = parseInt(
					parts.pop().replace("section", "")
				);
			}
		}
	...
	AccordionTabs.prototype._init = function () {
		...
		if (this.options.tabsAllowed && !this.isAccordion) {
